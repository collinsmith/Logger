...
static filename[PLATFORM_MAX_PATH];
...
public Logger:_CreateLogger(pluginId, numParams) {
    ...
    new loggerFile[PLATFORM_MAX_PATH];
    formatex(loggerFile, PLATFORM_MAX_PATH-1, "%s_%s.log", filename, szTime);
    
    if (loggerFiles == Invalid_Array) {
        loggerFiles = ArrayCreate(PLATFORM_MAX_PATH, INITIAL_LOGGER_SIZE);
    }
...
public _LoggerGetFile(pluginId, numParams) {
    ...
    new loggerIndex = convertLoggerToIndex(logger);
    new filenameLen = ArrayGetString(loggerFiles, loggerIndex, filename, PLATFORM_MAX_PATH-1);
